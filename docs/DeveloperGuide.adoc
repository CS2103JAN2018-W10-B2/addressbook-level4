...
//tag::Login/Logout[]
=== Login/logout & reset password feature
====Current Implementation
The login/logout command feature requires the sales person to enter password whenever he/she starts to use the addressbook. This will allow for a safer address book.
...
// tag::TotalPersonStatusBar[]
=== TotalPersonStatusBar feature
==== Current Implementation
This feature will allow user to see how many contacts he or she has in his/her address book in a straight way, so that I can call it straightforward when I need the information.
//end::TotalPersonStatusBar[]
...
=== Archive feature
==== Current Implementation
The archive feature allows the salesperson to archive clients that he no longer contacts, but does not want to delete as he possible might want to contact him/her again. Once archived, the client is not shown in the list. To display the client, either `find` or `listall` command has to be used. The client is then displayed in a red font with strikethrough, indicating that he/she is archived.
...
==== Ong Kuan Yang
* *Major*: A calendar or appointment scheduling feature allows the salesperson to record down when he has appointments with his clients and to see at a glance his future appointments for the days/month ahead.
...
==== Gloria Law
* *Major*: A mail server connected to the app which includes a mail panel that shows inbox and allows emails to be received and sent
...
|`* * *` |user |schedule meetings with my clients |remember when I am supposed to meet them

|`* * *` |user |tag clients with important info like the products they are interested in | so that I can list and group together related clients quickly.

|`* * *` |user |log notes specific to each client |so that I can recall the information when I need it
...
|`* *` |user |set the timezone for each contact |meet or call them at an appropriate time

|`* *` |user |archive certain contacts |useless or expired contacts won't clutter up the program but I can still find them when I need to

|`* *` |user |hide <<private-contact-detail,private contact details>> by default |minimize chance of someone else seeing them by accident

|`* *` |user |delete a certain tag for all persons |remove certain tags if the relationship is not there anymore
...
.  Should work with a large number of, which is more than 1000, contacts.
.  Should respond quickly after any action, which is less than 2 seconds.
.  Should store its data in plain text so that data can be manually accessed if required.
.  A user with above average typing speed for regular English text (i.e. not code, not system admin commands) should be able to accomplish most of the tasks faster using commands than using the mouse.
.  All actions should be performable using only the keyboard.
